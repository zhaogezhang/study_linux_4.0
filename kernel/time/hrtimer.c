...
/*********************************************************************************************************
** 函数名称: remove_hrtimer
** 功能描述: 尝试把指定的高精度定时器从其所属红黑树队列中移除
** 输	 入: timer - 指定的高精度定时器指针
**         : base - 指定高精度定时器的基准时钟
** 输	 出: 0 - 移除的高精度定时器为非激活状态
**         : 1 - 移除的高精度定时器为激活状态
** 全局变量: 
** 调用模块: 
*********************************************************************************************************/
...
** 功能描述: 根据函数指定参数启动指定的高精度定时器并触发高精度定时器 HRTIMER_SOFTIRQ 软中断
** 输	 入: timer - 指定的高精度定时器指针
**         : tim - 指定的高精度定时器超时时间间隔，单位 ns（表示从现在开始再过多长时间执行超时处理函数）
**         : mode - 指定的高精度定时器模式
** 输	 出: 1 - 启动成功
**         : 0 - 定时器处于激活状态
** 全局变量: 
...
/*********************************************************************************************************
** 函数名称: hrtimer_cancel
** 功能描述: 等待指定的高精度定时器执行完所有超时处理函数并关闭这个高精度定时器
** 输	 入: timer - 指定的高精度定时器指针
** 输	 出: 0 - 高精度定时器为非激活状态
**         : 1 - 高精度定时器为激活状态
** 全局变量: 
